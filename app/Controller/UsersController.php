...
class UsersController extends AppController {
    ...
    /**
     * show register view and redirect to student or teacher register's page
     */
    function register() {
        if ($this->request->is('post')) {
    ...
    /**
     * unlock user who was temp locked
     */
    ...
    /**
     * load 2 System Pram: TEMP_LOCK_TIME and MAX_TIME_WRONG_PASSWORD from database
     * @return array of SystemParam
     */
    function loadSystemParams() {
        ...
        if ($this->SystemParam->getNumRows() > 0) {
            return array(
                'TEMP_LOCK_TIME' => $sys[2]['SystemParam']['Value'],
                'MAX_TIME_WRONG_PASSWORD' => $sys[0]['SystemParam']['Value'],
                'AUTO_LOGOUT_TIME'  =>  $sys[1]['SystemParam']['Value'],
            );
        } else {
            return FALSE;
        }
     ...
     * @param int $lock_type    type of locking
     * 
     * @return boolean TRUE if lock success
     */
        ...
        if (!empty($data)) {
            ...
            if ($checkValidate) {
                if ($this->User->validateUser()) {
                    $dbSource = $this->User->getDataSource();
                    try {
                        if ($this->User->save($data, array('validate' => $checkValidate))) {
                            $dbSource->commit();
